...
.button-input {
	...
	transition: background-color 0.15s ease-in-out, border-color 0.15s
		ease-in-out, color 0.15s ease-in-out, box-shadow 0.15s ease-in-out;
	cursor: pointer;
	flex-shrink: 0;
	display: inline-flex;
	align-items: center;
...
%>
	...
	<main class="flex-1 p-5 overflow-y-auto max-w-7xl mx-auto">
		<div class="flex justify-between items-center mb-6">
			<h1 class="text-xl font-bold text-gray-800">Gestión de Cuentas
				Bancarias</h1>
			<a href="/TPINT_GRUPO_6_LAB4/TipoDeCuentaServlet?Param=precargaDdl"
		...
		<h3 class="text-lg font-semibold text-gray-800 mb-4">Filtros de
			Búsqueda</h3>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
					<div>
						<label for="tipoCuenta"
							...
							de Cuenta</label> <select id="tipoCuenta" name="tipoCuenta"
							class="mt-1 block w-full pl-3 pr-8 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 rounded-md">
				...
				<div>
					...
					<div class="flex items-end gap-3 h-10">
						<div class="relative flex-grow h-full">
							...
							<input type="text" id="buscarDni" placeholder="Ingrese el CBU"
								class="block w-full pl-10 pr-3 py-2 rounded-md leading-5 bg-white text-base input-glow-effect h-full">
						</div>
						<div class="flex gap-3 h-full">
							<input type="button" value="Buscar"
								class="button-input bg-red-600 text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-700 focus:ring-offset-2 w-28 h-full">
							<input type="button" value="Mostrar todo"
								class="button-input bg-gray-700 text-white hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 w-28 h-full">
						</div>
					</div>
				</div>
			</div>
				...
				<tbody class="bg-white divide-y divide-gray-200">
					...
					<tr>
						<td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
							...
							<button type="button"
								onclick="confirmarEliminar('<%=c.getNumeroCuenta()%>');"
								class="action-icon-button ml-2">
								<i class="fa-regular fa-trash-can fa-xl" style="color: #fa0000;"></i>
							</button>
						</td>
						<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><%=c.getNumeroCuenta()%></td>
						<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><%=c.getCBU()%></td>
						<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><%=c.getFechaCreacion()%></td>
						<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">$<%=c.getSaldo()%></td>
						<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><%=c.getTipoCuenta() != null ? c.getTipoCuenta().getDescripcion() : "N/A"%></td>
						<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><%=c.getCliente() != null ? c.getCliente().getNombre() + " " + c.getCliente().getApellido() : "N/A"%></td>
						<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><%=c.isActiva() ? "Activa" : "Inactiva"%></td>
					</tr>
					<%
					}
					}
		...
		<div class="flex justify-center items-center mt-5">
			<nav
				...
				<a href="?page=1"
					class="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-l-md hover:bg-gray-50 focus:z-20 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">1</a>
				<a href="?page=2" aria-current="page"
					class="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-red-600 bg-red-50 border border-red-500 z-10 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">2</a>
				<a href="?page=3"
					class="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 focus:z-20 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">3</a>
				<a href="?page=4"
					class="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 focus:z-20 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">4</a>
				<a href="?page=5"
					class="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-r-md hover:bg-gray-50 focus:z-20 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">5</a>
			</nav>
		</div>

		<a href="menuAdministrador.jsp"
			class="flex justify-end text-black underline">Volver</a>
	</main>
	...
	<form id="formEliminar" action="CuentaServlet" method="post"
		style="display: none;">
		<input type="hidden" name="action" value="eliminar"> <input
			type="hidden" name="cuentaIdEliminar" id="cuentaIdEliminar">
	</form>
